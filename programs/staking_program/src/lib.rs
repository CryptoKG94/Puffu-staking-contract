...
pub mod staking_program {
    ...
    pub fn initialize_staking_pool(
        ...
        let mut staking_pool = ctx.accounts.user_staking_pool.load_init()?;
        staking_pool.owner = ctx.accounts.owner.key();
        Ok(())
    }

    #[access_control(user(&ctx.accounts.user_staking_pool, &ctx.accounts.owner))]
    pub fn stake_nft_to_fixed(
        ...
        let mut staking_pool = ctx.accounts.user_staking_pool.load_mut()?;
...
*/
    ...
    pub fn claim_reward(
        ...
        let mut staking_pool = ctx.accounts.user_staking_pool.load_mut()?;
        let reward: u64 = staking_pool.claim_reward(
            timestamp
        )?;
...
pub struct InitializeStakingPool<'info> {
    #[account(zero)]
    pub user_staking_pool: AccountLoader<'info, UserPool>,
